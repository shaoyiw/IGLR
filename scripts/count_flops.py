...
from isegm.model.modeling.segformer.my_segformer_model import SegFormer
...
deeplab = DeepLabV3Plus(backbone='resnet101', ch=128, project_dropout=0.1, norm_layer=nn.BatchNorm2d,
...
model = DeeplabModel(backbone='resnet101', deeplab_ch=256, aspp_dropout=0.20, use_leaky_relu=True,
...
# flops_seg, params_seg = get_model_complexity_info(model, (3, 384, 384))
# # flops_refine, params_refine = get_model_complexity_info(refine_layer, (3,256,256))
# print('segmentor: ', flops_seg, params_seg)

model.to('cuda:1')
# 检查模型参数是否在GPU上
print(next(model.parameters()).device)  # 应输出 `cuda:3`

# 测量显存
model.eval()
torch.cuda.reset_peak_memory_stats()
input = torch.ones(1, 3, 384, 384).to('cuda:1')
output = model(input)
# output.sum().item()  # 强制计算
