...
class ISTrainer(object):
    ...
    def training(self, epoch):
        ...
        if self.is_master:
            ...
            if isinstance(self.checkpoint_interval, (list, tuple)):
                ...
                # [x for x in self.checkpoint_interval if x[0] <= epoch]筛选出所有 x[0]（起始 epoch）小于等于当前 epoch 的元组
                # [-1][1]选取最后一个元组的最后一个元素表示interval
                checkpoint_interval = [x for x in self.checkpoint_interval if x[0] <= epoch][-1][1]
            else:
                checkpoint_interval = self.checkpoint_interval

            if epoch % checkpoint_interval == 0 and epoch > 50:
                save_checkpoint(self.net, self.cfg.CHECKPOINTS_PATH, prefix=self.task_prefix,
        ...
        with torch.set_grad_enabled(not validation):
            ...
            with torch.no_grad():
                ...
                for click_indx in range(num_iters):
                    ...
                    if click_indx == 0:
                        prev_output = torch.sigmoid(eval_model(net_input, points, gate=0.0)['instances'])
                    else:
                        prev_output = torch.sigmoid(eval_model(net_input, points, gate=1.0)['instances'])
            ...
            if last_click_indx is not None:
                output = self.net(net_input, points, 1.0)
            else:
                output = self.net(net_input, points, 0.0)
    ...
    def save_visualization(self, splitted_batch_data, outputs, refine_output, global_step, prefix):
        ...
        predicted_refine_focus = draw_probmap(predicted_mask_focus)

        predicted_coarse_mask_focus = torch.sigmoid(refine_output['instances_coarse'])[0, 0].detach().cpu().numpy()
        pred_color_mask = np.zeros_like(images_focus)
        pred_color_mask[:, :, 0] = (predicted_coarse_mask_focus > 0.5) * 255
        predicted_coarse_mask_focus = draw_probmap(predicted_coarse_mask_focus)
        ...
        images_focus_with_points = add_tag(images_focus_with_points, 'Images Focus')
        predicted_refine_focus = add_tag(predicted_refine_focus, 'Refine Focus')
        predicted_coarse_mask_focus = add_tag(predicted_coarse_mask_focus, 'Coarse Focus')
        ...
        viz_image_focus = np.hstack(
            (images_focus_with_points, predicted_refine_focus, predicted_coarse_mask_focus, predicted_trimap_focus)).astype(
            np.uint8)
