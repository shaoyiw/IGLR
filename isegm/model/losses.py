...
class NormalizedFocalLossSigmoid(nn.Module):
    ...
    def forward(self, pred, label):
        ...
        # sample_weight表示有效的样本数，正样本+负样本
        if not self._from_logits:
            pred = torch.sigmoid(pred)
        # 平衡正负样本的alpha，这里正样本和负样本都设置为0.5
        alpha = torch.where(one_hot, self._alpha * sample_weight, (1 - self._alpha) * sample_weight)
        # 1.0 - torch.abs(label - pred)越接近1表示预测的越准确
        pt = torch.where(sample_weight, 1.0 - torch.abs(label - pred), torch.ones_like(pred))

        beta = (1 - pt) ** self._gamma  # 难样本的权重更大

        sw_sum = torch.sum(sample_weight, dim=(-2, -1), keepdim=True)  # 有效的样本个数
        beta_sum = torch.sum(beta, dim=(-2, -1), keepdim=True)  # beta的总和
        mult = sw_sum / (beta_sum + self._eps)  # 归一化因子
        if self._detach_delimeter:
            mult = mult.detach()  # 阻止梯度回传
        beta = beta * mult  # 调整之后的beta,为了防止某些样本的权重过大主导了模型训练
        if self._max_mult > 0:
...
class WFNL(nn.Module):
    ...
    def forward(self, pred, label, weight=None):
        ...
        if weight is not None:  # 如果有外部权重，则把外部权重和内部权重进行混合五五开
            weight = weight * self.w + (1 - self.w)
            loss = (loss * weight).sum() / (weight.sum() + self._eps)  # 加权平均
        else:
            bsum = torch.sum(sample_weight, dim=misc.get_dims_with_exclusion(sample_weight.dim(), self._batch_axis))
            loss = torch.sum(loss, dim=misc.get_dims_with_exclusion(loss.dim(), self._batch_axis)) / (bsum + self._eps)
        return loss
