...
def draw_with_blend_and_clicks(img, mask=None, alpha=0.6, clicks_list=None, pos_color=(0, 255, 0),
    ...
    result = img.copy()
    result_mask = np.tile(mask[:, :, np.newaxis], (1, 1, 3))
    if mask is not None:
    ...
    if clicks_list is not None and len(clicks_list) > 0:
        ...
        result_mask = draw_points(result_mask.astype(np.uint8) * 255, pos_points, pos_color, radius)
        result_mask = draw_points(result_mask, neg_points, neg_color, radius)
