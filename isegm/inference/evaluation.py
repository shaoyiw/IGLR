...
def evaluate_sample_with_certain_clicks(image, gt_mask, init_mask, predictor, max_iou_thr,
                                        pred_thr=0.49, min_clicks=1, max_clicks=10,
...
def evaluate_dataset_certain_click(dataset, predictor, vis=True, vis_path='./experiments/vis_val/', **kwargs):
    all_ious = []
    if vis:
    ...
    for index in tqdm(range(len(dataset)), leave=False):
        ...
        _, sample_ious, _ = evaluate_sample_with_certain_clicks(sample.image, sample.gt_mask, sample.init_mask,
                                                                predictor,
...
def evaluate_dataset(dataset, predictor, vis=True, vis_path='./experiments/vis_val/', **kwargs):
    ...
    for index in tqdm(range(len(dataset)), leave=False):
        ...
        _, sample_ious, sample_bious, sample_assds, _ = evaluate_sample(sample.image, sample.image_name, sample.gt_mask, sample.init_mask, predictor,
                                            sample_id=index, vis=vis, save_dir=save_dir,
                                            index=index, **kwargs)
        all_ious.append(sample_ious)
...
def evaluate_sample(image, image_name, gt_mask, init_mask, predictor, max_iou_thr,
                    pred_thr=0.49, min_clicks=1, max_clicks=20,
                    sample_id=None, vis=True, save_dir=None, index=0, callback=None,
                    progressive_mode=True, dilation_ratio=0.02,
                    ):
    clicker = Clicker(gt_mask=gt_mask)
    pred_mask = np.zeros_like(gt_mask)
    prev_mask = pred_mask
    ...
    with torch.no_grad():
        ...
        for click_indx in range(max_clicks):
            ...
            if click_indx == 0:
                # pred_probs = predictor.get_prediction(clicker)
                pred_probs = predictor.get_prediction_gate(clicker, gate=0.0)
            else:
                # pred_probs = predictor.get_prediction(clicker)
                pred_probs = predictor.get_prediction_gate(clicker, gate=1.0)
            pred_mask = pred_probs > pred_thr
        ...
        return clicker.clicks_list, np.array(iou_list, dtype=np.float32), np.array(biou_list,
