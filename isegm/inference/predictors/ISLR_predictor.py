...
class ISLRPredictor(object):
    ...
    def get_prediction(self, clicker, prev_mask=None):
        ...
        y1, y2, x1, x2 = get_click_crop(coarse_mask_np, prev_mask_np, global_roi, last_y, last_x, self.focus_crop_r)
        ...
        focus_pred = self._get_refine(pred_logits, image_full, clicks_list, feature, focus_roi,
                                      focus_roi_in_global_roi)  # .cpu().numpy()[0, 0]
        focus_pred = F.interpolate(focus_pred, (y2 - y1, x2 - x1), mode='bilinear',
                                   align_corners=True)  # .cpu().numpy()[0, 0]
    ...
    def get_prediction_gate(self, clicker, gate, prev_mask=None):
        clicks_list = clicker.get_clicks()
        click = clicks_list[-1]
        last_y, last_x = click.coords[0], click.coords[1]
        self.last_y = last_y
        ...
        pred_logits, feature = self._get_prediction_gate(image_nd, clicks_lists, is_image_changed, gate)
        prediction = F.interpolate(pred_logits, mode='bilinear', align_corners=True,
                                   size=image_nd.size()[2:])
        ...
        # Ablation Studies for only Target crop
        # self.prev_prediction = prediction
        # return prediction.cpu().numpy()[0, 0]
    ...
    def _get_prediction_gate(self, image_nd, clicks_lists, is_image_changed, gate):
        points_nd = self.get_points_nd(clicks_lists)
        output = self.net(image_nd, points_nd, gate)
        # output = self.net(image_nd, points_nd)
        return output['instances'], output['feature']
